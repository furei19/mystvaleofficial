...
void set_terminal_size(short width, short height) {
...
int main() {
    system(CLEAR_SCREEN);
    set_terminal_size(130, 38); // width x height
...
void blacksmith(struct player *player1, int place, char weapons[][100], int highlight, int type) {
    ...
    case 1:
        ...
        case 1:
            strncpy(line_1, temp[3], 110);
            strncpy(line_2, temp[3] + 110, 110);
...
void low_encounter(struct player *player1, int place) {
    ...
    // int encounter_type = 2; for testing
...
void show_health_bar(struct player *player1) {
    ...
    printf("Adventurer");
    printf("\033[5;23H");
    printf(GREEN"%d%%", player1->hp, RESET);
...
void check_quest(struct player *player1) {
    ...
    if (tier1_quest.activ == 1) {
        ...
        if (pass == 1 && tier1_quest.prog >= tier1_quest.req) {
            ...
            strcpy(tmp_msg, "You have completed the quest ");
            strcat(tmp_msg, tier1_quest.name);
            center_text(tmp_msg, 10);

            strcpy(tmp_msg, "You have been rewarded ");
